  ...
  .then(function (json) {
    for (var i in json) {
      ...
      if (json[i].url === "month") {
        ...
        insertElement += `<a href="${json[i].url}" target="_blank">`
        insertElement += `<img src="${json[i].image}" alt="イメージ画像">`
        insertElement += `<img src="${json[i].sub}" alt="対象生アイコン">`
        insertElement += `<h4 class="deadline">${json[i].deadline}</h3>`
        insertElement += `<h3 class="title">${json[i].title}</p>`
        insertElement += '</a></div>'
      }
      a++;
      b++;
    ...
    for (var j = 0; j < c; j++) {
      ...
      sortElement += `<a href="${json[sortdata[j][1]].url}" target="_blank">`
      sortElement += `<img src="${json[sortdata[j][1]].image}" alt="イメージ画像">`
      sortElement += `<img src="${json[sortdata[j][1]].sub}" alt="対象生アイコン">`
      sortElement += `<h4 class="deadline">${json[sortdata[j][1]].deadline}</h3>`
      sortElement += `<h3 class="title">${json[sortdata[j][1]].title}</p>`
      sortElement += '</a></div>'
    }
